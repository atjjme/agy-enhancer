...
│   ├── stop-service.bat           # 一键停止后台守护进程与设置微服务
│   ├── settings.bat               # 独立按需启动设置微服务并打开 settings.html
...
│   ├── loader.js                  # 核心守护进程与 Chrome DevTools Protocol 注入器
│   ├── settings-server.js         # 轻量按需设置微服务（仅在打开设置面板时运行，支持闲置自动关机）
...
| **`stop-service.bat`** | `scripts/` | **停止后台服务**：一键安全终止正在运行的 Node.js 守护注入进程与设置微服务，立刻释放所有资源。 |
| **`settings.bat`** | `scripts/` | **独立调出设置中心**：当后台已在运行时，如果想再次调出设置网页调整开关，双击此脚本即可瞬间按需拉起服务并打开浏览器。 |
...
无需在 Antigravity 界面内塞入繁杂难用的设置弹窗，也无需让后台常年开启耗费电脑资源的 HTTP 端口监听。
- **纯本地按需服务**：平常守护进程运行中 **0 开放端口、0 额外消耗**；
- **智能点亮引导**：新用户若直接用浏览器打开 `settings.html`，页面会友好显示：
  > 👋 **欢迎！您正在打开设置中心**  
  > 受浏览器安全限制，你需要双击运行 **install.bat** 以启动本服务，即可点亮本页面并开启全部功能！
  同时页面会自动处于待命探测状态。您只要双击根目录下的 **`install.bat`**，页面将**自动点亮**，即刻同步当前系统的真实配置，无需重新手动刷新！
- **分层级联动控制**：
  - **顶级总闸开关**：关闭时，整个功能区断电置灰休眠，客户端不挂载任何增强事件，100% 恢复官方原生纯净状态；
  - **右键菜单大开关与子开关**：可分别开启或关闭侧边栏菜单、消息代码块右键、图片右键；
  - **导航定位、项目归档、阅读记忆、智能未读、开机自启**均具备独立控制开关。
- **静默无感释放**：点击“保存设置”即可即时生效；配置完毕后直接关闭网页标签页即可，后台设置微服务会自动退出并释放 37210 端口，真正做到“用完即走，零残留占用”。
