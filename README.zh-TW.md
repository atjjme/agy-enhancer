...
### 📌 快速導航
[🚀 運行環境與安裝](#-運行環境與安裝) · [✨ 核心功能](#-核心功能與操作) · [🎨 參數自訂](#-參數自訂與熱更新) · [📜 油猴腳本](#-油猴腳本-userscript-指南) · [❓ 常見問題 FAQ](#-常見問題排查-faq) · [📦 立即下載](https://github.com/atjjme/agy-enhancer/releases/latest)
...
1. [指令碼生效：在 Antigravity 2.0 介面右上角顯示綠點指示](#tc-feature-1)
2. [快速定位：介面右下角顯示「向上」「向下」按鈕](#tc-feature-2)
3. [專案封存：未完成暫時不處理的專案可以一鍵封存](#tc-feature-3)
4. [右鍵選單：操作介面全面加入右鍵功能](#tc-feature-4)
5. [對話閱讀記憶：記住每個對話閱讀的進度](#tc-feature-5)
6. [智慧未讀判定：真實閱讀完輸出資訊後再標記為已讀](#tc-feature-6)

---

#### <a id="tc-feature-1"></a> 1. 指令碼生效：在 Antigravity 2.0 介面右上角顯示綠點指示
...
#### <a id="tc-feature-2"></a> 2. 快速定位：介面右下角顯示「向上」「向下」按鈕
...
#### <a id="tc-feature-3"></a> 3. 專案封存：未完成暫時不處理的專案可以一鍵封存
...
#### <a id="tc-feature-4"></a> 4. 右鍵選單：操作介面全面加入右鍵功能
...
| **實體檔案 / 制品卡片**<br>*(Artifact Card / 附件)* | 1. **`Reveal in Explorer`**<br>2. **`Copy Path`** *(僅所在目錄)*<br>*(圖片檔案額外支援: 3. `Copy Image`)* | • 在本機檔案總管中開啟並定位檔案所在目錄<br>• 複製該檔案所在目錄的絕對路徑（不带檔案名稱）<br>• 若卡片指向圖片檔案，支援直接複製圖片點陣圖 |
...
#### <a id="tc-feature-5"></a> 5. 對話閱讀記憶：記住每個對話閱讀的進度
...
#### <a id="tc-feature-6"></a> 6. 智慧未讀判定：真實閱讀完輸出資訊後再標記為已讀
